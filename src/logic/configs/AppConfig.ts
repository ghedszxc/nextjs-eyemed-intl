...
import { FORM_TRANSLATIONS } from "./constants/contact-us/FORM_TRANSLATIONS";
...
const AppConfig = (() => {
  ...
  const html = (str?: string) => {
    ...
    // const templated = setUriTemplates(newLine) || ""; // this function is outdated
    ...
    return parse(newLine?.replace(/os-/g, ""), options);
  };
  ...
  const getTranslatedSuccessMsg = (lang?: string) => {
    if (!lang) return "Missing lang";
    return FORM_TRANSLATIONS?.[lang]?.successMsg || "";
  };

  const getTranslatedPlaceholder = (lang?: string) => {
    if (!lang) return "Missing lang";
    return FORM_TRANSLATIONS?.[lang]?.textAreaMsg || "";
  };
