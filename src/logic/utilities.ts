...
export const getAdapterCTA = (obj: IAdapterCTAObj[]): ICta[] => {
  return obj?.map((link: IAdapterCTAObj): ICta => {
    const hash = link?.callToActionHash || link?.callToActionText;
    ...
    if (isFileDownload) {
      return {
        ...
        url: `/cap/content/${link?.target?.id}/?preview=true`,
...
export const getCoreMediaUrl = (str: string) => {
  ...
  return internalLink
    ? `${origin}${pathname.charAt(0) === "/" ? "" : "/"}${pathname}`
    : pathname;
};
